...
struct SessionRowView: View {
    ...
    var body: some View {
        HStack {
            ...
            if editMode?.wrappedValue == .inactive {
                // Single tap → confirmationDialog (action sheet from bottom)
                Button {
                ...
                .buttonStyle(.plain)
                .confirmationDialog("Options", isPresented: $showingActionSheet, titleVisibility: .hidden) {
                    Button("Save") {
                        CSVManager.exportSingleSessionAsCSV(session)
                    }
                    Button("Label") {
        ...
        .padding(.vertical, 4)
        // Row tap → navigate (original pattern kept)
        .background(
            NavigationLink("", destination: SensorGraphView(session: session))
                .opacity(0)
        )
        // Long press → contextMenu (same actions, richer UI)
        .contextMenu {
            Button {
                CSVManager.exportSingleSessionAsCSV(session)
            } label: {
